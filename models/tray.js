...
export default function main(params) {
  ...
  const boxHeightInMm = bottomFilletInMm * 2
  const backHeightInMm = gridSpacingInMm - boxHeightInMm - 2 * kerfInMm

  const box = createBox()
  const back = createBack()
  const leftSupport = createSupport().translateX(-wallThicknessInMm)
  const rightSupport = createSupport().translateX(-widthInMm)
  ...
  const shape = box
    ...
    .fillet(wallThicknessInMm / 3, (f) =>
      f.either([
        ...
        (f) => f.inPlane('YZ', 0),
        (f) => f.inPlane('YZ', -wallThicknessInMm),
        (f) => f.inPlane('YZ', -widthInMm),
        (f) => f.inPlane('YZ', -widthInMm + wallThicknessInMm),
      ]),
    )
  ...
  function createBox() {
    ...
    const wallProfile = draw()
      ...
      .vLine(bottomFilletInMm)
      .hLine(-wallThicknessInMm)
      .vLine(wallThicknessInMm / 2 - bottomFilletInMm)
      .customCorner(bottomFilletInMm - wallThicknessInMm)
      .lineTo([0, wallThicknessInMm])
      .done()

    const box = createProfileBox(wallProfile, base).translate(
      -bottomFilletInMm,
      bottomFilletInMm,
      0,
    )
  ...
  function createSupport() {
    return /** @type {Replicad.Solid} */ (
      draw()
        .lineTo([0, backHeightInMm - outerFilletInMm])
        .lineTo([depthInMm - 2 * bottomFilletInMm, 0])
        .close()
        .sketchOnPlane('YZ')
        .extrude(wallThicknessInMm)
        .translateZ(boxHeightInMm)
